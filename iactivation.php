...
<html lang="en">
  <head>
	...
	<?php
	  ...
	  $Email = $_GET['Email'];
	  $extract = mysqli_query($con,"SELECT * FROM institution where email='$Email' and activation='$activation'");
	  $count = mysqli_num_rows($extract);
	  if($count == 1)
	  {
	    $extract = mysqli_query($con,"UPDATE institution SET activation = '1' where email='$Email'");
